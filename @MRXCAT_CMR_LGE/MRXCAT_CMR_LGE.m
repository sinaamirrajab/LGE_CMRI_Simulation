%=========================================================================
% 
% MRXCAT_CMR_LGE MRXCAT class for LGE cardiac imaging
% 
% The MRXCAT_CMR_LGE class contains specific cine MRXCAT methods.
...
% 
%           LGEpar( MRX, filename )    External parameter function (LGEpar.m)
%                                       Use LGEpar.m to modify MRXCAT parameters
...
classdef MRXCAT_CMR_LGE < MRXCAT
    ...
    methods
        function MRX = MRXCAT_CMR_LGE( filename, filepath,varargin )
            ...
            for i = 1:length( varargin )
                ...
                if any( strcmpi( varargin{i}, {'subject'} ))
                    MRX.Par.scan.name = [MRX.Par.scan.name '_' varargin{i+1} '_'];
                end      
...
%             
            ...
            for t=MRX.Par.scan.phases % heart phases 
                for s=1:MRX.Par.scan.segments % k-space segments
                    ...
                    % ----------------------------------------------------------------
                    %   Low Pass Filter (Blur) - : msk needs to be re
                    %   sized to the size of the simulated data
                    %
                    % ----------------------------------------------------------------
...
%      mskn = imresize3(msk,size(KSPACE),'nearest');
     roi         = mskn==MRX.Par.act.myoLA_act|mskn==MRX.Par.act.myoLV_act| ... 
     mskn==MRX.Par.act.myoRA_act|mskn==MRX.Par.act.myoRV_act|...
     mskn==MRX.Par.act.bldplLA_act|mskn==MRX.Par.act.bldplLV_act|mskn==MRX.Par.act.bldplRA_act| ...
     mskn==MRX.Par.act.bldplLV_act;
            ...
            for i=1:length(act)
                ...
                switch char(tis(i))
                    ...
                    case {'myoLV_act','myoRV_act','myoLA_act','myoRA_act'}
                        rho = MRX.Par.tissue.rhomyomuscle;
                        t1  = MRX.Par.tissue.t1myomuscle;
                        t2  = MRX.Par.tissue.t2myomuscle;
                        % ------------------------------------------------------------
                        %   Blood
                        % ------------------------------------------------------------
                    case {'bldplLV_act','bldplLA_act'}
                        rho = MRX.Par.tissue.rhoblooda;
                        t1  = MRX.Par.tissue.t1blooda;
                        t2  = MRX.Par.tissue.t2blooda;
%                         t2s = MRX.Par.tissue.t2sblood;
%                         t2= 1/((1/t2s)-(1/t2));
                    case {'bldplRV_act','vein_activity','bldplRA_act','art_activity'}
                        rho = MRX.Par.tissue.rhobloodv;
                        t1  = MRX.Par.tissue.t1bloodv;
                        t2  = MRX.Par.tissue.t2bloodv;
                   ...
                   case {'r_kidney_cortex_activity','r_kidney_medulla_activity','l_kidney_cortex_activity','l_kidney_medulla_activity','adrenal_activity'}
                        rho = MRX.Par.tissue.rhokidney;
                        t1  = MRX.Par.tissue.t1kidney;
                        t2  = MRX.Par.tissue.t2kidney;
                   case {'spleen_activity'}
                        rho = MRX.Par.tissue.rhospleen;
                        t1  = MRX.Par.tissue.t1spleen;
                        t2  = MRX.Par.tissue.t2spleen;
                    case {'body_activity','esophagus_contents_activity','esophagus_activity'}
                        rho = MRX.Par.tissue.rhobody;
                        t1  = MRX.Par.tissue.t1body;
                        t2  = MRX.Par.tissue.t2body;
                    case {'small_intest_activity','desc_large_intest_activity','trans_large_intest_activity','asc_large_intest_activity','intest_air_activity','pancreas_activity'}
                        rho = MRX.Par.tissue.rhointestine;
                        t1  = MRX.Par.tissue.t1intestine;
                        t2  = MRX.Par.tissue.t2intestine;         
                        
                  
                    case 'muscle_activity'
                        rho = MRX.Par.tissue.rhoskmuscle;
                        t1  = MRX.Par.tissue.t1skmuscle;
                        t2  = MRX.Par.tissue.t2skmuscle;
                        
                    case {'st_wall_activity','st_cnts_activity','gall_bladder_activity'}
                        rho = MRX.Par.tissue.rhostomach;
                        t1  = MRX.Par.tissue.t1stomach; 
                        t2  = MRX.Par.tissue.t2stomach;
                        
                    case {'liver_activity','r_renal_pelvis_activity','l_renal_pelvis_activity'}
                    ...
                    case {'r_lung_activity','l_lung_activity'}
                        rho = MRX.Par.tissue.rholung;
                        t1  = MRX.Par.tissue.t1lung;
                        t2  = MRX.Par.tissue.t2lung;
                        
                        % --------------------------------------------------------
                        %   Lesion
                        % ------------------------------------------------------------
                    case {'scar1','scar2','scar3','scar4'}
                        rho = MRX.Par.tissue.rhoscar;
                        t1  = MRX.Par.tissue.t1scar;
                        t2  = MRX.Par.tissue.t2scar;  
                    
                    case {'bz1','bz2','bzin','bzout'}
                        rho = MRX.Par.tissue.rhobz;
                        t1  = MRX.Par.tissue.t1bz;
                        t2  = MRX.Par.tissue.t2bz; 
...
%                  E1 = exp(-tr./t1);
%                  E2 = exp(-tr./t2);
%                  sig = rho.*sin(a).*(1-E1).*sqrt(E2)*exp(-te/t2)./(1-(E1-E2).*cos(a)-E1.*E2); %sqrt(E2) <=> echo centered at TR/2 !
        ...
        function [img_rad,ksp] = radialResample( MRX, img )
            ...
            w    = MRXCAT_CMR_LGE.getRadWeights2D(samp,prof,0,ga,1);
            k    = MRXCAT_CMR_LGE.buildRadTraj2D(samp,prof,0,ga,1);
